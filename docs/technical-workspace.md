...
2. Set up the current managed global built-in skills: `argus-configure-invariant`, `argus-configure-workflow`, `argus-doctor`, `argus-setup`, `argus-intro`, and `argus-teardown`
...
`argus teardown --workspace <path>` removes one workspace registration. When the last workspace is removed, Argus also removes global hooks, global skills, global bootstrap artifacts, and the managed `~/.config/argus/` root.
...
- **Setup choice**: project-level setup writes skills to `.agents/skills/` and mirrors them into `.claude/skills/`. Argus intentionally does not create `.opencode/skills/` at project scope because OpenCode already supports compatible discovery. `argus setup --workspace` writes skills into agent-specific global skill directories, still using the Agent Skills format
...
- after upgrading the Argus binary, rerun `argus setup` to refresh the checked-in skills and prune obsolete built-in `argus-*` skill directories
...
#### Global Paths (written by `setup --workspace`)
...
Global scope currently distributes these managed built-in skills: `argus-configure-invariant`, `argus-configure-workflow`, `argus-doctor`, `argus-setup`, `argus-intro`, and `argus-teardown`.
