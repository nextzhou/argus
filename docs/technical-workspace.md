...
Both `install --workspace <path>` and `uninstall --workspace <path>` require confirmation unless `--yes` is provided. This includes rerunning `install --workspace <path>` for an already registered workspace, because refresh is still a mutating operation.
...
When `argus-project-init` fails, the injected guidance asks the agent to stop before answering the user's original task and to present the install / explain / ignore choices, preferably through the agent's question tool or another structured-choice input tool when available.
...
- `argus-intro`: explain what Argus is, why bootstrap reminders appear, and what installation changes
...
- after upgrading the Argus binary, rerun `argus install` to refresh the checked-in skills and prune obsolete built-in `argus-*` skill directories
