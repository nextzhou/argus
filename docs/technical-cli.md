...
| `argus install --workspace <path> [--yes] [--json]` | Register a workspace path. Install global `tick` hooks, global bootstrap skills, and global-scope artifacts under `~/.config/argus/`. If the workspace is already registered, rerunning the command refreshes those managed global resources to match the current binary. Confirmation is required unless `--yes` is used. Default output is text; `--json` returns a structured result. | `0` success, `1` failure |
