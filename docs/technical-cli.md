...
| 命令 | 说明 | 退出码 |
| :--- | :--- | :--- |
| `argus install [--yes]` | 安装 Argus 到当前项目。创建 `.argus/` 目录、配置 Agent Hook、生成内置 Skills。具有幂等性。子目录安装时需确认（`--yes` 跳过确认，详见 workspace §10.3.1）。 | 0: 成功, 1: 失败 |
| `argus install --workspace <path> [--yes]` | 注册 Workspace 路径。安装全局 Hook 和全局 Skills。Workspace 下未初始化的项目会被引导执行安装。执行前需确认（`--yes` 跳过）。 | 0: 成功, 1: 失败 |
| `argus uninstall [--yes]` | 从当前项目卸载 Argus。删除整个 `.argus/` 目录（Git-tracked 内容可通过 `git checkout` 恢复）、删除 Argus 管理的项目级 Skill 文件（`.agents/skills/argus-*` 与 `.claude/skills/argus-*`，保留非 `argus-` 前缀的用户自定义 Skill）、移除 Agent Hook 配置。执行前需确认（`--yes` 跳过）。Agent Skill 调用时应先检查 git 状态，提示用户未提交的文件，并使用 `--yes` 参数。 | 0: 成功, 1: 失败 |
| `argus uninstall --workspace <path> [--yes]` | 移除指定的 Workspace 路径。当没有剩余 Workspace 时，同时移除全局 Hook 和全局 Skills。执行前需确认（`--yes` 跳过）。 | 0: 成功, 1: 失败 |
| `argus doctor` | 诊断 Argus 配置状态（安装完整性、Hook 配置、文件校验等）。只诊断不治疗。 | 0: 通过, 1: 发现问题 |
| `argus version` | 显示版本号。 | 始终 0 |
...
| `argus job-done [--fail] [--end-pipeline] [--message "..."] [--markdown]` | Agent 报告当前 Job 结束。`--fail` 标记失败，`--end-pipeline` 提前结束 Pipeline（默认为成功，与 `--fail` 组合则为失败）。`--message` 附带摘要（可选）。默认 JSON，使用 `--markdown` 返回 Agent 友好格式。无活跃 Pipeline 时返回引导性提示。 |
| `argus status [--markdown]` | 主动查询项目综合概览（Pipeline 进度、Invariant 状态）。实时执行 invariant check。默认 JSON，使用 `--markdown` 返回 Agent 友好格式。 |
| `argus workflow start <workflow-id>` | 启动指定的 Workflow。Phase 1 强制单活跃：若已有 `status: running` 的 Pipeline，直接报错并提示先完成或取消当前 Pipeline。 |
| `argus workflow list` | 列出当前项目可用的所有 Workflow。 |
| `argus workflow cancel` | 中止当前活跃的 Pipeline。若存在多个 running Pipeline（异常状态），取消所有。无活跃 Pipeline 时返回 error envelope（`{"status": "error", "message": "no active pipeline"}`），exit 1。 |
| `argus workflow snooze --session <id>` | 在当前 Session 中暂时忽略活跃 Pipeline。`tick` 不再注入该 Pipeline 内容，直到新 Session 启动。无活跃 Pipeline 时返回 error envelope（同 cancel），exit 1。 |
| `argus workflow inspect [dir] [--markdown]` | 校验 Workflows 目录的格式与跨文件一致性。默认校验 `.argus/workflows/`。 |
| `argus invariant check [id]` | 执行 Invariant Shell 检查。若未指定 ID 则运行所有关联的检查。 |
| `argus invariant list` | 列出当前项目定义的所有 Invariants。 |
| `argus invariant inspect [dir] [--markdown]` | 校验 Invariants 目录的格式与交叉引用。默认校验 `.argus/invariants/`。 |
| `argus toolbox <tool> [args]` | 内置常用工具集（如 `jq`、`yq`），减少对宿主机环境的依赖。 |

## 7.3.1 生命周期命令成功输出

`install` / `uninstall` 家族命令成功时统一返回 JSON envelope：
...
具体命令的特定字段在实现时逐步定义，文档中已定义 `workflow start`、`job-done`、`status` 的详细 schema（见 8.2-8.4）。其余命令（`workflow list/cancel/snooze`、`invariant list/check/inspect`）遵循相同 envelope，Agent 对 JSON 结构有较强的适应能力。

### 常用命令规则

- **内部命令 (job-done, status 等)**：0 表示成功，1 表示业务错误（如参数无效、状态不匹配）。
- **外部命令**：
...
- **默认**：返回结构化 JSON，顶层包含 `"status": "ok/error"` envelope。
- **`--markdown`**（inspect/check）：返回格式化后的文本，供 Agent 直接解析或展示。

Phase 1 这些命令的具体字段定义在实现时根据实际需要确定，文档在实现阶段补充。当前仅保证统一 envelope 和 exit code 语义。
