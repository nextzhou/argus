...
### 7.1.1 作用域感知 (Scope Awareness)

Argus CLI 的所有命令（除明确不受限的命令外）均具有作用域感知能力。命令执行时通过 `scope.ResolveScope(cwd)` 逻辑自动解析当前工作目录所属的作用域：
- **项目作用域**：当当前工作目录 (CWD) 处于已初始化的项目内时。
- **全局/工作空间作用域**：当 CWD 处于已注册的 Workspace 路径下时。
- **无作用域**：不符合上述条件。此时大部分内部命令（如 `status`, `workflow` 等）将按需提示初始化。

这种模型确保了同一套命令集可以根据环境自动加载对应的编排产物（不变量、工作流、技能等）。
...
| `argus install --workspace <path> [--yes] [--json]` | 注册 Workspace 路径。安装全局 Hook、全局 Skills 以及全局作用域产物（不变量定义与目录结构至 `~/.config/argus/`）。运行时的具体编排语义请参考作用域模型文档。执行前需确认（`--yes` 跳过）。默认输出人类可读文本；`--json` 返回结构化结果。 | 0: 成功, 1: 失败 |
