...
本文档定义 Argus V2 CLI 的命令集、全局参数、退出码约定以及各场景下的输出格式。
...
| `argus install --workspace <path>` | 注册 Workspace 路径。安装全局 Hook 和全局 Skills。Workspace 下未初始化的项目会被引导执行安装。 | 0: 成功, 1: 失败 |
| `argus uninstall [--yes]` | 从当前项目卸载 Argus。删除整个 `.argus/` 目录（Git-tracked 内容可通过 `git checkout` 恢复）、删除 `.agents/skills/argus-*` 内置 Skill 文件（保留非 `argus-` 前缀的用户自定义 Skill）、移除 Agent Hook 配置。执行前需确认（`--yes` 跳过）。Agent Slash Command 中调用时应先检查 git 状态，提示用户未提交的文件，并使用 `--yes` 参数。 | 0: 成功, 1: 失败 |
| `argus uninstall --workspace <path>` | 移除指定的 Workspace 路径。当没有剩余 Workspace 时，同时移除全局 Hook 和全局 Skills。 | 0: 成功, 1: 失败 |
